    ...
    <div class="content">
      ...
      <section class="route is-active" id="route-home">
        ...
        <div class="table-wrap"><table>
          <thead><tr><th style="width:24%">Item</th><th>What's actually shipped</th></tr></thead>
          <tbody>
            <tr><td class="who">Mechanism in code</td><td>Real <code>bt.Synapse</code> types (v10.3.2) · real Glicko-2 · real FLORES-200 · real Yuma-style attack sim · real Solidity Speaker DAO.</td></tr>
            <tr><td class="who">6 LLM judges wired</td><td>Auto-selects Chinese-native first: <strong>GLM-4.6 · Qwen · Kimi · DeepSeek</strong>, then Claude Code (Max-sub CLI, no API key), then Anthropic API, then heuristic mock. Override with <code>LANGUAGEARK_JUDGE=…</code>.</td></tr>
            <tr><td class="who">On-chain DAO</td><td>Solidity contract deployed &amp; exercised on anvil. Validator reads miner Glicko ratings from <code>getRating()</code>, not JSON.</td></tr>
            <tr><td class="who">SOTA many-to-many MT</td><td>Miner wrappers for <strong>NLLB-200</strong> and <strong>SeamlessM4T-v2</strong> (Meta's 200-lang text + speech). <strong>FLORES+</strong> (2024, OpenLanguageData) as active eval benchmark.</td></tr>
            <tr><td class="who">Sponsor stack, end-to-end</td><td><strong>Alibaba</strong>: CosyVoice (TTS) · SenseVoice (ASR — covers nan/yue/hak/wu) · Qwen2-Audio (multimodal) · Qwen-max (judge). <strong>Zhipu</strong>: GLM-4.6 (judge) · GLM-4-Voice (speech LLM). One sponsor product per layer.</td></tr>
          </tbody>
        </table></div>
      </section>
      ...
      + <span style="color:#67e8f9">0.3</span>·<span style="color:#a78bfa">FLORES</span>   <span style="color:#64748b">// rotated held-out professional corpus</span></pre>
        ...
        <div class="table-wrap"><table>
          <thead><tr><th style="width:22%">Attack</th><th style="width:22%">Defence</th><th style="width:24%">Hyperparam</th><th>Why it works</th></tr></thead>
          <tbody>
            <tr><td class="who">Sybil speakers</td><td>2-of-3 DAO attestation + stake</td><td>100 TAO stake; on-chain Solidity, slashable</td><td>Costs more to fake an attestation than the payout for one Sybil vote.</td></tr>
            <tr><td class="who">Weight copying</td><td>Commit-reveal</td><td><code>commit_reveal_period = 5</code> tempos (~6 h)</td><td>By the time a copycat sees revealed weights, the eval set has rotated. See <a href="#/attack">attack sim →</a></td></tr>
            <tr><td class="who">Validator cabal</td><td>κ-clipping</td><td><code>kappa = 0.6</code> (default 0.5)</td><td>Cabal needs &gt;60 % of total stake to move consensus weights.</td></tr>
            <tr><td class="who">Single-miner cabal</td><td>Min weight spread</td><td><code>min_allowed_weights = 16</code></td><td>Each validator must distribute weight to ≥16 miners — no winner-take-all payouts.</td></tr>
            <tr><td class="who">Bond whipsaw</td><td>Tight liquid-α</td><td><code>α ∈ [0.05, 0.35]</code></td><td>Bonds can't swing faster than reality — small adversaries can't pump-and-dump.</td></tr>
            <tr><td class="who">Model copying</td><td>Proof-of-training</td><td>on-chain <code>(loss_curve_hash, dataset_hash)</code></td><td>Same model from two different "trainings" gets the same hash → caught.</td></tr>
          </tbody>
        </table></div>
      </section>
      ...
      <section class="route" id="route-lineage">
        ...
        <div class="table-wrap"><table>
          <thead><tr><th style="width:28%">Novelty</th><th>What none of the priors do</th></tr></thead>
          <tbody>
            <tr><td class="who">Stake-bonded speaker DAO</td><td>No existing subnet has an on-chain stake + attestation registry for human evaluators. Our Solidity contract is the org / anti-Sybil novelty.</td></tr>
            <tr><td class="who">3-signal composite</td><td>SN37 uses single-signal eval; SN1 uses single-signal LLM judge. We compose Elo + back-translation + held-out FLORES — fooling two requires fooling the third independently.</td></tr>
            <tr><td class="who">Cross-script Hokkien framing</td><td>SeamlessM4T-v2 has no <code>__nan__</code>; FLORES has only Latin-script <code>nan_Latn</code>. Building a real Han-script Hokkien corpus IS the deliverable.</td></tr>
          </tbody>
        </table></div>
      </section>
